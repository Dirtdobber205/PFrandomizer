class Primary {
	constructor(primary, optic, barrel, under, extra, ammo) {
	...
	toString() {
		return this.optic + ", " + this.barrel + ", " + this.under + ", " + this.extra + ", " + this.ammo;
	}
}

class Secondary {
	constructor(secondary, optic, barrel, extra, ammo) {
	...
	toString() {
		return this.optic + ", " + this.barrel + ", " + this.extra + ", " + this.ammo;
	}
}
...
function primaryGen(userRank) {
	const primaries = [
		...
		"000Intervention",
		"000M4A1",
		//"000PPK12", // Rank 0 in test place but this is more than likely to change.
		//"000HK51B", // Rank 0 in test place but this is more than likely to change.
		//"000SVK12E",// Rank 0 in test place but this is more than likely to change.
		"001KSG-12",
		"003MK11",
		"005UMP45",
		"006G36K",
	...
	let selection = selectionIterator(primaries, userRank);
	...
	const primary = new Primary(selection, opticAct, barrelAct, underAct, extraAct, ammoAct);

	console.log("Primary: " + primary.primary);
	console.log("Primary atts: " + primary.toString());
...
function secondaryGen(userRank) {
	...
	let selection = selectionIterator(secondaries, userRank);
	...
	const secondary = new Secondary(selection, opticAct, barrelAct, extraAct, ammoAct);

	console.log("Secondary: " + secondary.secondary);
	console.log("Secondary atts: " + secondary.toString());

	return secondary;
}

function randomLoadout() {
	let infoMessage = document.getElementById("info-msg");
	infoMessage.textContent = "";

	let primaryGun           = document.getElementById("priGun"),
		primaryAttachments   = document.getElementById("priAtts"),
		secondaryGun         = document.getElementById("secGun"),
		secondaryAttachments = document.getElementById("secAtts"),
		meleeWeapon          = document.getElementById("mel");

	const userRank = document.getElementById("rankInput").value;

	if (userRank < 0) {
		infoMessage.textContent = "Rank cannot be negative. No loadout generated.";
		primaryGun.textContent = "";
		primaryAttachments.textContent = "";

		secondaryGun.textContent = "";
		secondaryAttachments.textContent = "";
		meleeWeapon.textContent = "";
		
		throw new RangeError(`Expected userRank to be positive but found ${userRank}.`);
	} else {
		let generatedPri = primaryGen(userRank);
		let generatedSec = secondaryGen(userRank);
		let generatedMel = meleeGen();

		primaryGun.textContent = generatedPri.primary;
		primaryAttachments.textContent = generatedPri.toString();

		secondaryGun.textContent = generatedSec.secondary;
		secondaryAttachments.textContent = generatedSec.toString();

		meleeWeapon.textContent = generatedMel;
	}
}

function selectionIterator(type, userRank) {
	while (true) {
		let rawSelection = type[Math.floor(Math.random() * type.length)];
